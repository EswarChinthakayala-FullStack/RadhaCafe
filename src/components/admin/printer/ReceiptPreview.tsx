...
export function ReceiptPreview({ order, cafeSettings }: ReceiptPreviewProps) {
  ...
  return (
    <div className="receipt-preview w-full max-w-[340px] mx-auto bg-card text-foreground p-5 pb-8 my-2 rounded-md border border-border/80 shadow-md font-mono text-xs leading-relaxed select-none space-y-2.5">
      ...
      <div className="text-center space-y-0.5">
        <h3 className="font-bold text-sm text-foreground tracking-tight uppercase font-heading">{receipt.cafeName}</h3>
      ...
      <div className="space-y-1 text-[11px]">
        ...
        {receipt.tax > 0 && (
          <div className="flex justify-between text-muted-foreground">
            <span>GST Tax</span>
            <span className="font-mono text-foreground">{formatCurrency(receipt.tax)}</span>
          </div>
        )}
        {receipt.discount > 0 && (
