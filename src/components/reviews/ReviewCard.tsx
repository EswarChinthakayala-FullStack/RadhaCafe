...
import { StarIcon, CheckmarkCircle02Icon } from '@hugeicons/core-free-icons';
...
export function ReviewCard({
  ...
  return (
    <Card
      className={`rounded-md transition-all duration-300 flex flex-col justify-between ${isFeatured
          ? 'bg-[#25140D] border-2 border-[#E5A88B]/60 shadow-xl'
          : 'bg-[#1D100A] border border-[#2C1810] hover:border-[#E5A88B]/40 shadow-md hover:-translate-y-1 hover:shadow-lg'
        } ${className}`}
    >
      <CardContent className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-bold text-sm text-cream">{customerName}</span>
                {isVerified && (
                  <span
        ...
        {/* Message */}
        <p className="text-xs sm:text-sm text-cream/80 leading-relaxed font-normal italic">
          &ldquo;{message}&rdquo;
        </p>

        {/* Optional Featured Tag */}
        {isFeatured && (
          <div className="pt-2 border-t border-[#3E2519]/60 flex items-center justify-between">
