...
import { useWaterCustomers } from '../../../hooks/useWaterCustomers';
import { formatCurrency } from '../../../lib/utils/formatCurrency';
import { formatDate } from '../../../lib/utils/formatDate';
import type { WaterCustomer } from '../../../types';
import { WaterCustomerFormModal } from '../../../components/admin/water/customers/WaterCustomerFormModal';
...
export function WaterCustomersPage() {
  ...
  const [selectedPayCustomer, setSelectedPayCustomer] = useState<WaterCustomer | null>(null);

  const { data, isLoading, isError, error } = useWaterCustomers({
    page,
    limit: 50,
    search: searchQuery,
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex items-start sm:items-center gap-2.5">
          ...
          <div>
            <h2 className="text-lg sm:text-2xl font-bold font-heading text-foreground tracking-tight">
              Water Customers & Credit Ledger
            </h2>
            <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 leading-tight sm:leading-normal">
              Manage water customer profiles, order history, and Pay-Later credit balances (Isolated from Cafe).
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-card border border-border/80 p-3 rounded-md shadow-2xs">
        <div className="relative w-full sm:w-72">
          ...
          />
          <Input
            placeholder="Search by water customer name or phone..."
            value={searchQuery}
            onChange={(e) => {
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center bg-card rounded-md border border-destructive/20 text-destructive space-y-2">
          ...
          <p className="font-bold text-sm">Failed to load water customers</p>
        ...
        <div className="p-12 text-center bg-card rounded-md border border-dashed border-border/80 space-y-3">
          <HugeiconsIcon icon={UserGroupIcon} className="mx-auto w-10 h-10 text-muted-foreground/40" />
          <p className="font-bold text-sm text-foreground">No water customers found</p>
          <Button
            size="sm"
            onClick={() => setShowAddModal(true)}
